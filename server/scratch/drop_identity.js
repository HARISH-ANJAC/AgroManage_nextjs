...
const configs = [
    ...
    { schema: 'stoentries', table: 'TBL_DELIVERY_FILES_UPLOAD', column: 'SNO' },
    { schema: 'stoentries', table: 'TBL_SALES_PROFORMA_FILES_UPLOAD', column: 'SNO' },
    { schema: 'stoentries', table: 'TBL_SALES_ORDER_FILES_UPLOAD', column: 'SNO' },
    { schema: 'stoentries', table: 'TBL_CUSTOMER_RECEIPT_FILES_UPLOAD', column: 'SNO' },
    { schema: 'stoentries', table: 'TBL_TAX_INVOICE_FILES_UPLOAD', column: 'SNO' },
    { schema: 'stoentries', table: 'TBL_EXPENSE_FILES_UPLOAD', column: 'SNO' },
    { schema: 'stoentries', table: 'TBL_GOODS_FILES_UPLOAD', column: 'SNO' },
    ...
    { schema: 'stomaster', table: 'CUSTOMER_CREDIT_LIMIT_FILE_UPLOAD', column: 'SNO' },
    { schema: 'stomaster', table: 'TBL_CUSTOMER_MASTER_FILES_UPLOAD', column: 'SNO' },
];
...
async function dropIdentity() {
    ...
    try {
        for (const config of configs) {
            ...
            try {
                process.stdout.write(`Dropping identity from ${schema}.${table}.${column}... `);
                await client.query(`ALTER TABLE "${schema}"."${table}" ALTER COLUMN "${column}" DROP IDENTITY IF EXISTS;`);
                console.log(`Success.`);
            } catch (err) {
                console.log(`Failed: ${err.message}`);
            }
        }
        console.log("All identity properties dropped.");
    } catch (err) {
