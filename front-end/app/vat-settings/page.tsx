...
export default function VatSettingsPage() {
  ...
  return <MasterCrudPage
    domain="vat" title="VAT Percentage Settings" description="Manage your VAT percentage settings" idPrefix="VAT" fields={[
      ...
      { key: "effectiveFrom", label: "Effective From", type: "date", formatter: (val: any) => val ? new Date(val).toISOString().split('T')[0] : "" },
      { key: "effectiveTo", label: "Effective To", type: "date", formatter: (val: any) => val ? new Date(val).toISOString().split('T')[0] : "" },
